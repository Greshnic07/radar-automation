...
# Тянем прокси из секретов Гитхаба
...
def run():
    ...
    if PROXY_URL:
        ...
        print("⚠️ Прокси не задан, идем напрямую (велик шанс бана!)")
    ...
    with sync_playwright() as p:
        ...
        # Подключаем прокси, если он есть
        if PROXY_URL:
        ...
        for name, url in LOCATIONS_YANDEX.items():
            try:
                ...
                # Жмем "Ещё", чтобы развернуть тексты
                page.evaluate("""() => {
    ...
    # Оставляем только по 2 штуки для ТВ, чтобы файл не распух
    data["gis"] = data["gis"][:2]
